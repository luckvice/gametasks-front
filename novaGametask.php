      ...
      <h1 class="my-4">Nova Gametask
        <small>novo</small>
          ...
          <input type="hidden" value="N" id="id_usuario" name="id_usuario"/>
				  ...
				  <select name="id_jogo_task" id="id_jogo_task" required placeholder="Selecione um jogo"> <option>Selecione</option> </select>
					<small id="jogohelp" class="form-text text-muted"></small>
				  </div>
				  <div class="form-group">
					<label for="platafomra">Selecione uma Plataforma</label>
          <br>
				  <select id="id_plataforma_task" name="id_plataforma_task" required><option>Selecione</option> </select>
					<small id="plataformahelp" class="form-text text-muted"></small>
				  </div>
				  <div class="form-group">
					<label for="titulo">Status do jogo</label>
          <br>
				  <select id="status"><option value="-1">Selecione</option> <option value="0">Jogando</option>  <option value="1">Dando um tempo</option>  <option value="2">Rejogando</option>  <option value="3">Finalizado</option>  </select>
    ...
    console.log(key, val);
	   ...
	   $("#salvar").on('click', function(){
        ...
        console.log("WHHHHATTT?:"+$("#GametaskForm").serialize());
            // send ajax
            $.ajax({
                url: 'http://localhost:8080/api/novaGameTask', // url where to submit the request
                type : "POST", // type of action POST || GET
                dataType : 'json', // data type
                data : $("#GametaskForm").serialize(), // post data || get data
